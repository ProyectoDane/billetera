...
function getTotales(billetes) {
  return billetes.reduce(
    (acum, current) => {
      let cantBilletes =
        acum.cantBilletes + (current.isCoins ? 0 : 1 * current.quantity);
      let cantMonedas =
        acum.cantMonedas + (current.isCoins ? 1 * current.quantity : 0);
      return {
...
function pagarConRecursivo(amount, myMoney) {
  ...
  if (totalEnBilletera < amount) return [];
    ...
    if (moneyEntry.amount >= amount) {
      ...
      if (veces >= 1) {
        ...
        if (vuelto === 0) {
          ...
          let ret = pagarConRecursivo(
            amount - moneyEntry.amount * cantidadBilletes,
            myMoney.slice(index + 1),
          );
          //Ret tiene un monton de resultados posibles, cada resultado se le agrega al resultado parcial actual

          for (const unRet of ret) {
            let billetes = [
              copyMoney(moneyEntry, cantidadBilletes),
              ...(unRet.billetes || []),
            ];
...
function sortResultados(resultados) {
  resultados.sort((a, b) => {
    if (a.vuelto < b.vuelto) return -1;
    if (a.vuelto > b.vuelto) return 1;

    if (a.totales.cantBilletes < b.totales.cantBilletes) return -1;
    if (a.totales.cantBilletes > b.totales.cantBilletes) return 1;
    return 0;
  });
