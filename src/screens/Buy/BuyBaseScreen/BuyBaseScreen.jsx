...
const BuyBaseScreen = ({
  ...
  const schema = BalanceSchema(initialTotal);
  ...
  const handleContinue = async () => {
    ...
    try {
      let substract = 0;
      let newCoins = [...initialCoins];
      let newBills = [...initialBills];
      for (let opt of optPay) {
        substract += opt.amount * opt.quantity;
        await deleteMoney(opt.userId, opt.moneyId, opt.quantity);
        if (opt.quantity > 0) {
      ...
      vuelto
        ? toastNotification(`ACORDATE DE CARGAR TU VUELTO DE ${vuelto} EN LA BILLETERA !`, 'info', 'info')
        : toastNotification('LA COMPRA SE REALIZÓ CORRECTAMENTE!', 'success', 'success');
          ...
          <FormProvider {...methods}>
            ...
            >
              <View style={{flexDirection: 'row', marginTop: 10}}>
                ...
                <Text style={styles.resultTextAmount}>{initialTotal > 0 ? formatNum(initialTotal) : `$0`}</Text>
              </View>
            </View>
            <View
              style={{
            ...
            >
              ...
              >
                ...
                >
                  ...
                  >
                    <Text style={styles.resultTextLabel}>VALOR DE LA COMPRA:</Text>
                    <Text style={styles.resultTextAmount}>{formatNum(valueBuy)}</Text>
                  </View>
                  <View
                    style={{
                      flexDirection: 'row',
                  ...
                  >
                    <Text style={styles.resultTextLabel}>FORMA DE PAGO SUGERIDA:</Text>
                    <Text style={styles.resultTextAmount}>{formatNum(cantidadPagar)}</Text>
                  </View>
                  {optionBill ? (
                      ...
                      <View style={{marginRight: 10}}>
                        {optionBill.map((item, index) => (
                          <View key={index} style={styles.itemOptBuy}>
                            <Text style={{flex: 5, ...styles.text}}>{item}</Text>
                            <ItemMoney
                              key={'money_' + index}
                              {...optPay[index]}
                              style={{
                  ...
                  {vuelto ? (
                    ...
                    >
                      <Text style={{...styles.resultTextLabel, fontWeight: 'bold'}}>TU VUELTO:</Text>
                      <Text style={styles.resultTextAmountVuelto}>{vuelto}</Text>
                    </View>
                  ) : null}
                </View>
              </View>
