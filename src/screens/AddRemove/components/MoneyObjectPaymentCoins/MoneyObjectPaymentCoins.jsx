...
const MoneyObjectPaymentCoins = (elem) => {
  const totalStock = elem.quantity;
  const [total, setTotal] = useState(elem.quantity);
  const { index, coins, setCoins, total: total2, setTotal: setTotal2 } = elem;

  const add = () => {
    setTotal(total + 1);
    coins[index].quantity = coins[index].quantity + 1;
    setTotal2(total2 + coins[index].amount);
    setCoins(coins);
  };
